...
{{-- ! approval finance to sales order --}}
<script>
    $(document).ready(function() {
        Echo.channel('approval_notif').listen('ApprovalMessage', (e) => {
            let sound_approv = new Audio("{{ asset('sounds/so.wav') }}");
            sound_approv.play();
            let csrf = $('meta[name="csrf-token"]').attr("content");
            $.notify({
                title: e.title,
...
{{-- ! verification sales order for verificator (Admin Sales) --}}
@if ($verificate_so == 1)
    <script>
        $(document).ready(function() {
            Echo.channel('realtimeNotif').listen('SOMessage', (e) => {
                let soundPO = new Audio("{{ asset('sounds/so.wav') }}");
                soundPO.autoplay = true;
                soundPO.play();
                let csrf = $('meta[name="csrf-token"]').attr("content");
                $.notify({
                    title: e.title,
...
@endif
{{-- ! approval superadmin for purchase order --}}


@if ($approve_leave == 1)
    <script>
        $(document).ready(function() {
            Echo.channel('leave-approval').listen('LeaveApproval', (e) => {
                let soundPO = new Audio("{{ asset('sounds/so.wav') }}");
                soundPO.autoplay = true;
                soundPO.play();
                let csrf = $('meta[name="csrf-token"]').attr("content");
