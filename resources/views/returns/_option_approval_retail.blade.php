@if ($return->retailBy->directSalesCreditBy->count() <= 0)
    <a href="#" class="fw-bold text-success modal-btn2" href="#" data-bs-toggle="modal"
        data-original-title="test" data-bs-target="#editReturn{{ $return->id }}">
        {{ $return->return_number }}</a>
@else
    @php
        $cust_name = '';
        if (is_numeric($return->retailBy->cust_name)) {
            $cust_name = $return->retailBy->customerNumericBy();
        } else {
            $cust_name = $return->retailBy->cust_name;
        }
        $num_order = $return->retailBy->order_number;
    @endphp
    <a href="{{ url('retail/manage_payment?' . 'cancel_cust=' . $cust_name . '&cancel_order=' . $num_order) }}"
        ...
        <div class="modal-content">
                ...
                <div class="modal-header">
                    <h6 class="modal-title" id="exampleModalLabel">
                        ...
                        {{-- <div>
                            <small class="fw-bold text-danger">* Don't forget to cancel the payment!</small>
                        </div> --}}
                    </h6>
                </div>
                <div class="modal-body">
                  <form action="{{ url('return_retail/' . $return->id . '/approve_return_retail') }}" method="POST"
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            ...
                            <div class="form-group col-12 col-lg-3 other" hidden>
                                <label for="">&nbsp;</label>
                                <textarea name="return_reason" class="form-control" rows="1" placeholder="Write Your Reasons Here..."></textarea>
                            </div>
                            <div class="form-group col-12 col-lg-4">
